...
import 'abstract_error_correction_test_case.dart';
...
void main(){
  ...
  final List<int> PDF417_TEST = [
      ...
      900, 900];
  final List<int> PDF417_TEST_WITH_EC = [
      48, 901, 56, 141, 627, 856, 330, 69, 244, 900, 852, 169, 843, 895, 852, 895, 913, 154, 845, 778, 387, 89, 869,
      901, 219, 474, 543, 650, 169, 201, 9, 160, 35, 70, 900, 900, 900, 900, 900, 900, 900, 900, 900, 900, 900, 900,
  ...
  final int ECC_BYTES = PDF417_TEST_WITH_EC.length - PDF417_TEST.length;
  final int ERROR_LIMIT = ECC_BYTES;
  final int MAX_ERRORS = ERROR_LIMIT ~/ 2;
  final int MAX_ERASURES = ERROR_LIMIT;
  ...
  test('testMaxErrors', (){
    ...
    for (int testIterations = 0; testIterations < 100; testIterations++) { // # iterations is kind of arbitrary
      List<int> received = PDF417_TEST_WITH_EC.toList();
      AbstractErrorCorrectionTestCase.corrupt(received, MAX_ERRORS, random);
      checkDecode(received);
    }
  });
  ...
  test('testTooManyErrors', () {
    List<int> received = PDF417_TEST_WITH_EC.toList();
    Random random = AbstractErrorCorrectionTestCase.getRandom();
    AbstractErrorCorrectionTestCase.corrupt(received, MAX_ERRORS + 1, random);
    try {
      checkDecode(received);
